...
- `.specs/project/HARNESS.md` — o que guia e o que mede o agente, e o que
  deliberadamente não existe — **atualize quando mudar um controle**.
...
**The `HARNESS.md` line is required whenever that file exists** (harness-engineer
writes it; path per its step 3, so adapt `.specs/project/` to whatever this
project resolved). It carries an *obligation*, not just a location: the harness
record is living memory, and a change that alters what guides or measures the
agent must update it. Keep the imperative — without it the pointer degrades into
a file nobody maintains, which is worse than absent because it reads
authoritative while describing a harness that no longer exists.
