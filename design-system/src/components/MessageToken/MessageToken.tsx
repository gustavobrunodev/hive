 ...
 */
export function MessageToken({ kind, icon, className, children, ...rest }: MessageTokenProps) {
  const glyph = icon === undefined && kind === "command" ? <SlashGlyph /> : icon
  return (
    <mark className={cx("hds-message-token", className)} data-kind={kind} {...rest}>
      {glyph}
      {children}
    </mark>
  )
