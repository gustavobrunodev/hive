 ...
 */
export function useScrollLockEscape(node: HTMLElement | null): void {
  useEffect(() => {
    if (node === null) return
    const escape = (event: Event): void => {
      event.stopPropagation()
    }
    // `touchmove` for the same reason: the lock cancels it on the same rule,
    // and an Electron window on a touch laptop is a supported way to run this.
