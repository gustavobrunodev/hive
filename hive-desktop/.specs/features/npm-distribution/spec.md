...
- **ND-R3.1** The app resolves the tarball URL for **its own platform+arch**
  ...
  `registry.npmjs.org` (never a CDN — ND-C1).
- **ND-R3.2** Download progress is streamed as a percentage, with enough
  information for the UI to show transferred/total bytes.
- **ND-R3.3** The downloaded tarball is **verified against the registry's
  `dist.integrity` (sha512)** before anything is extracted or executed. A
  mismatch aborts, deletes the artifact, and reports a distinct integrity
  error — it is never silently retried or ignored.
