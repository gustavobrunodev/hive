...
function deps(overrides: Partial<AudioDeps> = {}): AudioDeps {
  return {
    decode: async () => fakeBuffer([[0.5, -0.5, 0.25]], WHISPER_SAMPLE_RATE),
...
describe('whisper/audio', () => {
  ...
  describe('browserAudioDeps', () => {
    ...
    it('renders through an OfflineAudioContext sized for the target rate', async () => {
      const rendered = fakeBuffer([[0.1, 0.2]], WHISPER_SAMPLE_RATE)
      const start = vi.fn()
      const connect = vi.fn()
      ...
      const out = await browserAudioDeps().resample(source, WHISPER_SAMPLE_RATE)

      expect(OfflineCtor).toHaveBeenCalledWith(1, 16000, WHISPER_SAMPLE_RATE)
    ...
    it('never asks for a zero-length render (guards a degenerate duration)', async () => {
      const OfflineCtor = vi.fn(() => ({
        ...
        destination: {},
        startRendering: async () => fakeBuffer([[0]], WHISPER_SAMPLE_RATE)
      }))
      vi.stubGlobal('OfflineAudioContext', OfflineCtor)

      await browserAudioDeps().resample(fakeBuffer([[]], 48000), WHISPER_SAMPLE_RATE)
      expect(OfflineCtor).toHaveBeenCalledWith(1, 1, WHISPER_SAMPLE_RATE)
    })
  })

  describe('downmixToMono', () => {
    it('returns the single channel untouched for mono input', () => {
      const mono = downmixToMono(fakeBuffer([[0.1, 0.2]], WHISPER_SAMPLE_RATE))
  ...
  describe('decodeToWhisperPcm', () => {
    it('produces mono Float32 PCM at the Whisper sample rate', async () => {
      const pcm = await decodeToWhisperPcm(blobOfSize(64), deps())
    ...
    it('resamples when the source rate differs, and skips the pass when it matches', async () => {
      ...
      await decodeToWhisperPcm(
        blobOfSize(64),
        deps({ decode: async () => fakeBuffer([[0.1, 0.2, 0.3]], 44100), resample })
      )
      expect(resample).toHaveBeenCalledWith(expect.anything(), WHISPER_SAMPLE_RATE)

      // Already 16 kHz → no resample at all.
      resample.mockClear()
      await decodeToWhisperPcm(blobOfSize(64), deps({ resample }))
      expect(resample).not.toHaveBeenCalled()
    })

    it('downmixes a stereo source on the way through', async () => {
      const pcm = await decodeToWhisperPcm(
        blobOfSize(64),
        deps({
          decode: async () =>
            fakeBuffer(
    ...
    it('rejects an empty file with a typed error', async () => {
      const error = await decodeToWhisperPcm(blobOfSize(0), deps()).catch((e) => e)
    ...
    it('rejects undecodable audio with a typed error (SB-R4.6)', async () => {
      const error = await decodeToWhisperPcm(
        blobOfSize(64),
        deps({
          decode: async () => {
    ...
    it('rejects a decode that yields zero samples', async () => {
      const error = await decodeToWhisperPcm(
        blobOfSize(64),
        deps({ decode: async () => fakeBuffer([[]], WHISPER_SAMPLE_RATE) })
      ).catch((e) => e)
      expect((error as AudioDecodeError).kind).toBe('empty')
    })
    ...
    it('rejects an all-zero (silent) take instead of returning an empty transcript', async () => {
      const error = await decodeToWhisperPcm(
        blobOfSize(64),
        deps({ decode: async () => fakeBuffer([[0, 0, 0, 0]], WHISPER_SAMPLE_RATE) })
      ).catch((e) => e)
      expect(error).toBeInstanceOf(AudioDecodeError)
