...
import { t } from '../../i18n'
import { AudioDecodeError, decodeToAsrPcm } from '../../asr/audio'
...
export function audioErrorMessage(error: unknown): AudioFailure {
  ...
  // The case that used to live here — the WASM heap running out, where "tente
  // de novo" was bad advice because the next attempt hit the same ceiling — is
  // gone with the heap. Native ONNX Runtime frees its memory, so a retry is
  // honest advice and the generic sentence is the right one.
  return {
    ...
    try {
      for (;;) {
        ...
        try {
          ...
          const pcm = await decodeToAsrPcm(blob)
