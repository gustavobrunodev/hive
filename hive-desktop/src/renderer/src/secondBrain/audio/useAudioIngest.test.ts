...
import { AudioDecodeError } from '../../asr/audio'
import type { AsrEngine } from '../../asr/useAsr'
...
function engine(transcribe = vi.fn().mockResolvedValue('transcrito')): AsrEngine {
  return {
    phase: { status: 'idle' },
    transcribe,
...
function setup(asr = engine()): {
  ...
  const { result } = renderHook(() => useAudioIngest(asr, onTranscript))
  return {
...
describe('useAudioIngest (T17)', () => {
  ...
  it('decodes then transcribes, handing the text to the shared field', async () => {
    ...
    const { add, onTranscript, asr, queue } = setup()

    add([item('reuniao.wav')])

    await waitFor(() => expect(onTranscript).toHaveBeenCalledWith('transcrito', 'reuniao.wav'))
    expect(asr.transcribe).toHaveBeenCalledWith(pcm, {
      // The partial stream: the job row is wired to show words as they decode
      // rather than a spinner that says nothing. The engine no longer streams
      // them (sherpa decodes in one call), but the wiring stays because it is
      // what the row reads, and what a streaming engine would fill.
  ...
  it('is the SAME path for a recorded Blob as for an uploaded File', async () => {
    decodeToAsrPcm.mockResolvedValue(new Float32Array([0.1]))
    ...
    expect(decodeToAsrPcm).toHaveBeenCalledTimes(2)
  })
  ...
  it('reports each decode failure with copy that says what went wrong (SB-R4.6)', async () => {
    ...
    for (const [kind, message] of cases) {
      vi.clearAllMocks()
      decodeToAsrPcm.mockRejectedValue(new AudioDecodeError(kind, 'boom'))
