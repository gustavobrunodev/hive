...
import { Alert, Button, Panel, Progress, Spinner } from '@hive/design-system'
import { t } from '../i18n'
 ...
 * Spinner label, `done` calls `onComplete()` straight away (so an
 * already-up-to-date workspace resolves fast without extra noise, R4.3),
 * `error` shows an Alert with both a retry button and a "continue anyway"
 * button (R4.2 — the work UI must not be permanently blocked by a failed
 * update). Unlike `GuidedInstall` (T9), this never renders a step list: an
 * update is a background-ish gate, not a first-run ceremony.
 */
          ...
          {phase.status === 'error' ? (
            <>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                ...
                <Button onClick={onComplete}>{t('updateGate.continueAnywayCta')}</Button>
              </div>
            </>
          ) : (
            <>
              <Progress value={null} />
              <Spinner label={progressMessage ?? t('updateGate.progressLabel')} />
            </>
          )}
        </Panel>
      </div>
